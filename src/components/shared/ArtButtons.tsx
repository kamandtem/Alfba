...
export const CloseArt: React.FC<{ onClick: () => void; className?: string; label?: string }> = ({ onClick, className = '', label = 'بستن' }) =>
  <button type="button" className={`art-btn art-close ${className}`} onClick={onClick} aria-label={label}>
    <img src="/assets/ui/btn-close.svg" alt="" draggable={false} />
  </button>;
